...
import { FackCallOfDutyApi } from './../../Apis/FackCallOfDutyApi';
import CallOfDutyBox from '../../components/Card/CallOfDutyBox';

const CallOfDuty = () => {
    document.title = "sweez | Call-Of-Duty"
    const [cpApi, setCpApi] = useState(FackCallOfDutyApi)
    DaimonSelected()
    return (
        <>
            <MainBox>
                ...
                <form>
                    <DivBox>
                        <Headling index={1} text={"Call of duty account info"} />
                        <div className="p-2">
                    ...
                    <DivBox>
                        ...
                        <div className='grid lg:grid-cols-2 gap-4 my-2'>
                            {
                                cpApi.map(({ id, cp, price, available }) => {
                                    return (
                                        <CallOfDutyBox key={id} cp={cp} price={price} available={available} text={"CP"} />
                                    )
                                })
                            }
                        </div>
